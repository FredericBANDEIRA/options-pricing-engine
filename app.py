"""
Derivatives Pricer — Streamlit Entry Point

A professional-grade derivatives pricing engine featuring:
...
- Exotic options (barrier, Asian, digital)
- Multi-leg option strategies
- Volatility surface (SVI + yfinance market data)
- Fixed-income bond pricing (YTM, duration, convexity, DV01)
- Interactive Plotly charts and quant interview questions
"""
...
from pricer.ui.tab_bonds import render_bonds_tab
...
@st.cache_data
def _cached_early_exercise(
    S: float, K: float, T: float, r: float, q: float, sigma: float, opt: str
) -> dict:
...
# ---------------------------------------------------------------------------
st.set_page_config(
    page_title="Financial Instruments Pricer",
    page_icon="📈",
    layout="wide",
...
# ---------------------------------------------------------------------------
st.markdown(
    '<div class="app-header">Financial Instruments Pricer</div>', unsafe_allow_html=True
)

# ---------------------------------------------------------------------------
# Tabs
# ---------------------------------------------------------------------------
tab_pricer, tab_strategies, tab_vol, tab_bonds, tab_interview = st.tabs(
    [
        "📊 Derivatives Pricer",
        "📈 Strategies",
        "🌊 Vol Surface",
...
with tab_pricer:
    ...
    with st.expander("📐 Gamma PnL Calculator", expanded=False):
        ...
        with col_move:
            st.markdown(
                '<div class="param-label">Spot move %</div>', unsafe_allow_html=True
            )
            spot_move = st.number_input(
                "Spot move %",
                value=1.0,
                step=0.25,
                format="%.2f",
    ...
    with st.expander("🇺🇸 Early Exercise Analysis — American Style", expanded=False):
        if T > 0:
            ...
            if ee["early_exercise_premium"] > 0.001:
                st.info(
                    f"Early exercise premium of **{ee['early_exercise_premium']:.4f}** "
                    f"({ee['premium_pct']:.2f}% of European price). "
                    f"American-style pricing is relevant for this configuration."
                )
            else:
                st.success(
                    "Early exercise premium is negligible. "
                    "European and American prices are essentially equal."
                )
        else:
            st.warning(
                "Option has expired (T = 0). No early exercise analysis available."
            )
    ...
    with st.expander("🔍 Implied Volatility Solver", expanded=False):
        ...
        with col1:
            market_price = st.number_input(
                "Market Price", value=float(bs.price(S, K, T, r, q, sigma, opt))
            )
        with col2:
    ...
    with st.expander("🎲 Monte Carlo Engine", expanded=False):
        st.markdown(
            "Price this option using a vectorised Monte Carlo simulation with antithetic paths and a control variate."
        )
        if st.button("Run Simulation (100k paths)"):
            with st.spinner("Simulating..."):
                mc_res = price_european_mc(
                    S, K, T, r, q, sigma, opt, n_paths=100000, control_variate=True
                )
                ...
                st.caption(
                    f"95% Confidence Interval: [{mc_res['ci_lower']:,.4f}, {mc_res['ci_upper']:,.4f}]"
                )
    ...
    with st.expander("🌴 Exotic Options (same parameters)", expanded=False):
        ...
        if "Barrier" in ex_type:
            c1, c2 = st.columns(2)
            barrier_lvl = c1.number_input(
                "Barrier Level", value=float(S * 1.2 if opt == "call" else S * 0.8)
            )
            b_type = c2.selectbox(
                "Barrier Type",
            ...
            st.metric(
                f"{b_type.replace('-', ' ').title()} {opt.title()}", f"${bp:,.4f}"
            )
    ...
    with col_h1:
        st.markdown(
            '<div class="chart-section-title">vs Spot</div>', unsafe_allow_html=True
        )
    with col_h2:
        st.markdown(
            '<div class="chart-section-title">vs Volatility</div>',
...
with tab_interview:
    ...
    st.markdown(
        "Test your knowledge with these derivatives and quantitative finance questions."
    )
    ...
    for cat, items in categories.items():
        st.markdown(
            f'<div class="interview-category">{cat}</div>', unsafe_allow_html=True
        )
